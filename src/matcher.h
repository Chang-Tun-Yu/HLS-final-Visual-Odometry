...
private:
  ...
  // void  myCreateIndexVector (int32_t m[MAX_FEATURE_ARRAY_SIZE],int32_t& n,int32_t k[BIN_NUM][MAX_FP_IN_BIN], int32_t k_num[BIN_NUM],const int32_t &u_bin_num,const int32_t &v_bin_num, int32_t m_num[BIN_NUM]);
  // inline void myFindMatch (int32_t m1[MAX_FEATURE_ARRAY_SIZE],const int32_t &i1,int32_t m2[MAX_FEATURE_ARRAY_SIZE],const int32_t &step_size,int32_t k2[BIN_NUM][MAX_FP_IN_BIN], int32_t k2_num[BIN_NUM], 
...
//   void removeOutliers (Vector<Matcher::p_match> &p_matched,int32_t method);
  ...
  // int32_t max2c[MAX_FEATURE_ARRAY_SIZE];
  // int32_t max2p[MAX_FEATURE_ARRAY_SIZE];
  int32_t* max2p, *max2c;
